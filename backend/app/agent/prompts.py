...
def _build_system_section(turn: AgentTurnContext) -> str:
    ...
    base_rules = [
        ...
        "工具会按当前阶段真实裁剪；只能使用本轮可见工具。如果缺少目标工具，先用阶段推进或回滚工具切到正确阶段。",
        ...
        "如果用户一开口就是想找菜、推荐晚餐、看冰箱做什么，就应该在同一轮里完成进入推荐阶段和搜索/推荐动作，而不是只口头说会处理。",
        "用户点名想吃或想做某一道具体菜时，例如“西红柿炒鸡蛋”“鱼香肉丝怎么做”，必须优先调用 search_recipes：query 传用户原始完整菜名，ingredients 传能从菜名中明确拆出的食材。",
        "指定菜名搜索顺序必须是：完整菜名 -> 食材 -> 标签/步骤文本。search_recipes 未找到时，如实告诉用户数据库暂未收录，不能直接文字现编菜谱内容。",
        "只有用户明确表示“可以现写”“帮我现写一份”“那你新建一个”等同意后，才可以调用 create_or_update_task_recipe 并传 recipe 创建生成菜谱快照。",
        "recommend_recipes 只用于开放式推荐或按已有食材/偏好找菜；不要用它替代具体菜名搜索。",
        "create_or_update_task_recipe 复制数据库菜谱时只传 recipe_id；现写菜谱时只传完整 recipe。recipe_id 和 recipe 不能同时传。",
        "推荐阶段只做局部微调时，用 update_task_recipe_for_planning，不要为了改一个食材或步骤重建整份菜谱。",
    ...
    stage_rules = {
        ConversationStage.IDLE: [
            ...
            "当前阶段重点工具：start_recommendation_task、search_recipes、recommend_recipes、create_or_update_task_recipe。用户明确想找菜或做菜时，可以在同一轮进入推荐中并继续搜索/推荐；create_or_update_task_recipe 仅限用户已明确同意现写。",
        ],
        ConversationStage.RECOMMENDING: [
            "当前处于推荐中阶段，核心目标是帮用户从数据库选定一道菜；只有用户明确同意现写时，才现场创建一道新菜。",
            "可使用推荐、图片识别、创建/覆盖任务菜谱、展示菜谱详情、推进到备料、取消任务等工具。",
            "当前阶段重点工具：search_recipes、recommend_recipes、create_or_update_task_recipe、update_task_recipe_for_planning、show_recipe_detail_card、recognize_image_ingredients、advance_to_preparation、cancel_recommendation_task。",
            "用户点名一道菜时先 search_recipes，不要直接 create_or_update_task_recipe(recipe=...)；search_recipes 未找到时，先询问是否需要现写。",
            "用户要求调整当前任务菜谱的食材、步骤、份量、耗时、标签或小贴士时，用 update_task_recipe_for_planning 做局部更新。",
            "如果用户只是想看看某道菜的详情，可以先展示菜谱详情卡，不一定立刻覆盖当前任务菜谱。",
            "如果当前没有任务菜谱，就不能推进到备料阶段。",
        ],
        ConversationStage.PREPARING: [
            "当前处于备料中阶段，核心目标是帮助用户把食材准备状态推进完整。",
            "优先根据用户描述或图片识别结果更新食材状态、补充说明，必要时展示备菜清单卡。",
            "当前阶段重点工具：update_task_recipe_for_preparation、show_pantry_card、recognize_image_ingredients、advance_to_cooking、rollback_to_recommendation、cancel_preparation_task。",
            "只有在必需食材都备齐后才能推进到烹饪阶段。",
            "如果用户改主意想换菜，可以回滚到推荐中或取消任务。",
            "如果用户说“这些还差什么”“我现在备到哪了”，优先依据当前任务菜谱快照里的食材状态回答；除非客户端卡片状态明确指出别的焦点，否则不要擅自改写备料进度。",
            "如果用户上传食材图片，先 recognize_image_ingredients；需要写入准备状态时再调用 update_task_recipe_for_preparation。",
